...
<!--       action="?a=product_c&dir=3" -->
        <button onclick="KiemTraXoa()" class="sh_action_link" id="btn_delete_prd_ok">
           <p>Xóa</p>  
        </button>
        ...
        <div class="confirm_delete_prd" id="content_confirm_delete_prd">
          ...
          <form action="?a=product_c&dir=3" accept-charset="utf-8" method="post" onsubmit="return XacNhanXoa()" style="float: left; margin-right: 15px;">
              <input type="text" id="get_id_prd" name="delete_id_watch" style="display: none">
              <button type="submit" onsubmit="return XacNhanXoa()" style="background: #65b688">Xác nhận</button>
          </form>
...
<div class="col-xs-12">
    <div class="table-responsive sh_prd_table_manager">
        <table class="sh_table_list_order" width="100%">
            ...
            <tr>
                <td>
                    <input id="prd_<?php echo $value_kq['id_watch']?>" name="prd_item_<?php echo $value_kq['id_watch']?>" type="checkbox" class="sh_btn_checkbox" value="<?php echo $value_kq['id_watch']?>" onclick="DeleteProductId(<?php echo $value_kq['id_watch']?>)">
