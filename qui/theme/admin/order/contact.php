<!-- <?php print_r($list_arr['num_rows']);
echo count($list_arr['list_contacts']);
?> -->
<div class="row">
        ...
        <div class="sh_list_order">
            ...
            <div class="row" style="margin: 0">
                <div class="table-responsive sh_content_list_new_order">
                    <table class="sh_table_list_order" width="100%">
                        ...
                        <?php
                        $dem = 1;
                        foreach ($list_arr['list_contacts'] as $list_ct) {?>
                          <tr width="100%">
                              <td><?php echo $dem++?></td>
                              <td><a href="?a=order&pages=detail&dir=contact&id=<?php echo $list_ct['id_contact_us']?>"><?php echo $list_ct['name_cus_contact_us']?></a></td>
                              <td><?=$list_ct['date_contact_us']?></td>
                              <td><?=$list_ct['info_contact_us']?></td>
                              <td><?php
                                  if($list_ct['viewed_contact_us'] == 0){
                                      echo "<span>Chưa</span>";
                                  }else{
            ...
            <div class="sh_pagination p12">
              <ul>
                  ...
                  ?>"><li>Trở về</li></a>
                  <a href="<?php echo $current;
                    if(!isset($_GET['trang']) && $list_arr['num_rows'] - 8 <= 0){
                        echo 1;
                    }else{
                        if(!isset($_GET['trang']) && $list_arr['num_rows'] - 8 > 0){
                            echo 2;
                    ...
                    if(isset($_GET['trang']) && $list_arr['num_rows'] - $_GET['trang']*8 > 0){
                        echo $_GET['trang'] + 1;
                    }else{
                        if(isset($_GET['trang']) && $list_arr['num_rows'] - $_GET['trang']*8 <= 0){
                            echo $_GET['trang'];
